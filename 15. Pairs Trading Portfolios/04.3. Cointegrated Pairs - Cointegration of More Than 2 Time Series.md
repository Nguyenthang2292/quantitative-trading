...
The R packages `urca` and `egcm` implement a long list of stationarity and cointegration tests (Clegg, 2023; Pfaff et al., 2022). ↩︎
...
Morgan Stanley Capital International (MSCI) is a leading provider of investment decision support tools and services. The company is best known for its global equity indices, which are widely used by investors to benchmark and analyze the performance of equity markets around the world.↩︎