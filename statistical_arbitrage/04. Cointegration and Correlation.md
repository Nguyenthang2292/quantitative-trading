...
- Biểu thức này cho thấy hệ số tương quan *(correlation)* có thể giảm xuống rất thấp nếu phương sai *(variance)* của các thành phần phần dư *(residual)* *(- variances of the residual terms)* $$\sigma_{1t}^2, \sigma_{2t}^2$$ được chọn thích hợp so với $$\sigma_t^2$$.
...
Dù hai chuỗi có thể đồng tích hợp *(cointegration)* *(cointegration)* mạnh (có xu hướng hội tụ về một trạng thái cân bằng dài hạn), nhưng tương quan *(correlation)* tức thời giữa các thay đổi ngắn hạn của chúng có thể rất thấp hoặc thậm chí không liên quan.
#### 2. Ví dụ thực tế:
...
### Kết luận:
Trong chiến lược *pairs trading*, điều quan trọng là *cointegration* chứ không phải *correlation*, vì trọng tâm là tính hồi quy về trung bình dài hạn chứ không phải sự đồng biến ngắn hạn.
